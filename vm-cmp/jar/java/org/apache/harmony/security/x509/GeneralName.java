...
public class GeneralName {
    ...
    public static void checkURI(String uri) throws IOException {
        try {
            ...
            if ((ur.getScheme() == null) 
                ...
                throw new IOException("Bad representation of uniformResourceIdentifier. It must include the scheme and a scheme-specific-part: " + uri);
            }
            if (!ur.isAbsolute()) {
                throw new IOException("Bad representation of uniformResourceIdentifier. It should not be relative: " + uri);
            }
        } catch (URISyntaxException e) {
            throw (IOException) new IOException(
                    "Bad representation of uniformResourceIdentifier." + uri).initCause(e);
    ...
    public static int[] oidStrToInts(String oid) throws IOException {
        ...
        if (bytes[bytes.length-1] == '.') {
            throw new IOException("The OID: '" + oid + "' is incorrect");
        }
        ...
        if (isIPv4) {
            ...
            while (i < ip_bytes.length) {
                ...
                if (digits == 0) {
                    ...
                    throw new IOException("Incorrect IP representation: " + ip);
                }
            ...
            for (int i=0; i<ip_bytes.length; i++) {
                ...
                if ((bytik >= '0') && (bytik <= '9')) {
                    ...
                    if (component % 2 == 1) {
                        ...
                        throw new IOException("Incorrect IPv6 representation: '" + ip + "'"); //$NON-NLS-1$
                    }
                    if (bytik == '/') {
                        if (reading_second_bound) {
                            // more than 2 bounds in the range
