...
/*
** $Id: oswald.h,v 1.7 2006/10/04 14:24:20 cvsroot Exp $
*/
...
/*
** We're using POSIX threads for oswald like functions and types
*/
...
/*
** pthreads based x_Monitor and the x_monitor_... primitives.
*/
...
/*
** pthreads based x_Queue and the x_queue_... primitives.
*/
x_status x_queue_create(x_queue queue, void *queue_start, w_size queue_size);
...
/*
** pthreads based x_Sem and the x_sem_... primitives.
*/
x_status x_sem_create(x_sem semaphore, w_size initial_count);
x_status x_sem_delete(x_sem semaphore);
x_status x_sem_get(x_sem semaphore, x_sleep wait_option);
x_status x_sem_put(x_sem semaphore);
x_status x_sem_wait(x_sem semaphore);
x_status x_sem_post(x_sem semaphore);

/*
** pthreads based x_Mutex and the x_mutex_... primitives.
*/
...
/*
** And again for threads.
*/

x_status x_thread_create(x_thread thread, x_entry entry_function, void* entry_input, void *stack_start, w_size stack_size, w_size priority, w_word flags);
...
#ifdef DEBUG
void *_x_mem_alloc(w_size bytes, const char *file, int line);
void *_x_mem_calloc(w_size bytes, const char *file, int line);
...
#else
void *_x_mem_alloc(w_size bytes);
void *_x_mem_calloc(w_size bytes);
...
*/
w_ubyte* x_os_main(int argc, char** argv, w_ubyte *first_unused_memory);
