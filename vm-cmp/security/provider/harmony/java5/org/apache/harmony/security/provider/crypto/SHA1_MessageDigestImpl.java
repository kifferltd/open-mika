...
/*
 * Imported by CG 20090513 based on Apache Harmony ("enhanced") revision 490473.
 */
...
public class SHA1_MessageDigestImpl extends MessageDigestSpi 
    ...
    protected int engineDigest(byte[] buf, int offset, int len) throws DigestException { 
        ...
        if ( offset > buf.length || len > buf.length || (len + offset) > buf.length ) {
            throw new IllegalArgumentException("bad length/offset");
        }
        if ( len < DIGEST_LENGTH ) {
            throw new DigestException("length < 20");
        }
        if ( offset < 0 ) {
