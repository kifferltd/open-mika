...
/* Imported 20090910 by CG based on Apache Harmony "advanced" revision 764827 */
...
public class X509CertFactoryImpl extends CertificateFactorySpi {
    ...
    private byte[] decodePEM(InputStream inStream, byte[] boundary_suffix) 
        ...
        if (boundary_suffix == null) {
            ...
            if (ch != '\n') {
                throw new IOException("Incorrect PEM encoding: New line code is expected after the opening delimiter boundary.");
            }
        }
        ...
        if (buff[index-1] != '\n') {
            throw new IOException("Incorrect Base64 encoding: New line code is expected before closing delimiter boundary.");
        }
        // check and skip closing boundary delimiter prefix
        // (first '-' was read)
        for (int i=1; i<pemClose.length; i++) {
            if (pemClose[i] != inStream.read()) {
                throw new IOException("Incorrect PEM encoding: '-----END"
                        + ((boundary_suffix == null) 
                            ? "" 
                            : new String(boundary_suffix))
    ...
    private static Certificate getCertificate(byte[] encoding) 
        ...
        if (encoding.length < CERT_CACHE_SEED_LENGTH) {
            throw new CertificateException("Input Stream contains not enough data.");
        }
    ...
    private static CRL getCRL(byte[] encoding) 
        ...
        if (encoding.length < CRL_CACHE_SEED_LENGTH) {
            throw new CRLException("Input Stream contains not enough data.");
        }
