...
w_driver_status con_write(w_device device, w_ubyte *bytes, w_int length, w_int *lwritn, x_sleep timeout) {
  ...
  while (rem_length > 0) {
    ...
    Font_drawStringAligned(cons->buffer, cons->font, cons->x, 0, screen->width - cons->x, cons->font->size * 2, ascii2String(rem_bytes, this_time), R_WHITE, (unsigned int)ALIGNMENT_LEFT);
