...
/* $Id: rudolph.c,v 1.5 2006/06/09 08:34:20 cvs Exp $ */
...
#  error Sorry, but the code in awt/rudolph/hal/hostos/linux/xsim/rudolph.c is not generic enough to handle any pixel type yet, only 4 and 16bpp.  So you should hack the aforementioned source file to support your new pixel size, and define a suitable XSIM_HACK_BPP constant, or (better still) take the hack out and make the code in the X simulator a bit more generic.  If this makes you shake your fist at the sky, I hereby promise to buy you a beer for not finishin g the job properly.
#endif

#ifndef RUDOLPH_DEFAULT_FRAME_TITLE
#define RUDOLPH_DEFAULT_FRAME_TITLE "Mika"
#endif

#ifndef RUDOLPH_DEFAULT_FRAME_WIDTH
#define RUDOLPH_DEFAULT_FRAME_WIDTH 400
#endif

#ifndef RUDOLPH_DEFAULT_FRAME_HEIGHT
#define RUDOLPH_DEFAULT_FRAME_HEIGHT 240
#endif
...
static x_Monitor xLock;

x_monitor xlock = &xLock;
;
  // NOTE: a lot of static variables to speed up the simulator
  //       and because X-programming is so 'clumpsy'.

void set_frame_title(char *title) {
  XTextProperty windowName;
  XTextProperty *pWindowName = &windowName;
  XTextProperty iconName;
  XTextProperty *pIconName = &iconName;
  
  if (!XStringListToTextProperty((char **) &title, 1, pWindowName)) {
    pWindowName = NULL;
  }
  
  if (!XStringListToTextProperty((char **) &title, 1, pIconName)) {
    pIconName = NULL;
  }

  XSetWMName(display, window, pWindowName);
  XSetWMIconName(display, window, pWindowName);
}
...
static void x_create_window(void) {
  ...
  char *window_name = RUDOLPH_DEFAULT_FRAME_TITLE;
  char *icon_name = RUDOLPH_DEFAULT_FRAME_TITLE;
...
r_screen screen_init() {
  w_int width = RUDOLPH_DEFAULT_FRAME_WIDTH;
  w_int height = RUDOLPH_DEFAULT_FRAME_HEIGHT;
...
void screen_update(int x1, int y1, int x2, int y2) {
  ...
  rc = x_monitor_enter(xlock, x_eternal);
