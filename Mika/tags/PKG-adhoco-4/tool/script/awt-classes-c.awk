...
#
# $Id: awt-classes-c.awk,v 1.4 2006/06/09 09:51:06 cvs Exp $
# 
...
# second was a field.
  ...
  else if(substr($2,1,1)!="(") realfields[thisclazz,thismember] = "F_"thisclazz"_"thismember
  else {
    method=thismember
    descr=$2
...
END {
  ...
  print "/*"
  print "** w_clazz declarations for the AWT classes"
  print "*/"
  print " "
  ...
  #print "/*"
  #print "** slot numbers of Java or native methods"
  #print "*/"
  #print " "
  #for(cm in offset) {
  #  printf "w_int %s;\n", offset[cm]
  ...
  printf("/*\n** Wotsit functions for awt-classes\n*/\n\n");
  for(c = 1; c in clazz; ++c) {
    thisclazz = clazz[c];
    mcount = 0;
    for (m in methods) {
    ...
    #printf("void fixup_%s(w_clazz clazz) {\n\n", thisclazz);
    #for (cf in realfields) {
    #  split(cf, a, SUBSEP)
    #  if(a[1] == thisclazz) {
    #    printf "  %s = findFieldOffset(clazz, \"%s\");\n", realfields[cf],a[2]
  ...
  for(c = 1; c in clazz; ++c) {
    ...
    #printf "        slashed = cstring2String(\"%s\",%d);\n",string,length(string)
    #printf "        dotified = slashes2dots(slashed);\n"
    #printf "        woempa(1,\"Adding {%%w,%%p} to fixup_hashtable\\n\",dotified,fixup_%s);\n",thisclazz
    #printf "        deregisterString(slashed);\n"
    #printf "        ht_write_no_lock(fixup_hashtable,(w_word)dotified,(w_word)fixup_%s);\n",thisclazz
  }
  print "}"

  print  "w_clazz loadOneAWTClass(w_string name) {"
...
#  printf  "void loadAWTClasses() {\n\n"
#  print  "  w_string classname;"
#  print  "  w_string dotified;"
#  print  "  "
#
#  for(c = 1; c in clazz; ++c) {
#    string = sprintf("%s%s", path[c], clazz[c]);
#    thisclazz = clazz[c]
#    printf("  classname = cstring2String(\"%s\", %d);\n", string, length(string));
#    printf "  dotified = slashes2dots(classname);\n"
#    printf "  clazz%s = seekClazzByName(dotified, NULL);\n", thisclazz
#
#   printf "  if (clazz%s==NULL) {\n",thisclazz
#    printf "    clazz%s = loadBootstrapClass(dotified);\n",thisclazz
#    printf "    if (clazz%s==NULL) {\n",thisclazz
#    printf "      woempa(9,\"Unable to find AWT class %%w\\n\",classname);\n"
#    printf "    }\n"
#    print  "  }"
#    print  "  deregisterString(dotified);"
#  }
