...
import java.awt.AWTPermission; // at least the class you are testing ...
import java.security.*;
import java.util.Enumeration;

/**
*  this file contains test for java.security.AWTPermission   <br>
*
*/
public class AcuniaAWTPermissionTest implements Testlet
{
  ...
  public void test (TestHarness harness)
    {
       th = harness;
       th.setclass("java.awt.AWTPermission");
       test_AWTPermission();
       test_equals();
       test_hashCode();
       test_implies();
...
*
*/
  public void test_AWTPermission(){
    th.checkPoint("AWTPermission(java.lang.String)");
    String s = "acunia.*";
    AWTPermission bp = new AWTPermission(s);
    th.check(bp.getName() , s , "checking name");
    s = "*";
    bp = new AWTPermission(s);
    th.check(bp.getName() , s , "checking name");
    try {
        new AWTPermission("");
    ...
    try {
        new AWTPermission(null);
        th.fail("should throw a NullPointerException -- 7");
    }
    catch(NullPointerException iae) { th.check(true , "caught exception -- 7"); }

    th.checkPoint("AWTPermission(java.lang.String,java.lang.String)");
    try {
	bp = new AWTPermission("cool.*", null);
	th.check(bp.getName() , "cool.*");
	new AWTPermission("cool.*", "");
	new AWTPermission("cool.*", "*ddf");	
	th.check(true);
    }
...
*/
  public void test_equals(){
    ...
    AWTPermission bp = new AWTPermission("com.acunia");
    th.check(!bp.equals(null), "null is allowed");
    th.check(!bp.equals(new AWTPermission("com.*")), "not equal -- 2");
    th.check(bp.equals(new AWTPermission("com.acunia","1")), "equal");
...
*/
  public void test_implies(){
    ...
    AWTPermission bp1 = new AWTPermission("com.acunia");
    AWTPermission bp2 = new AWTPermission("com.*");
...
*/
  public void test_newPermissionCollection(){
    ...
    pc.add(bp);
    pc.add(new AWTPermission("be.*"));
    pc.add(new AWTPermission("vm.smartmove"));
    pc.add(new AWTPermission("com.acunia.vm*"));
    ...
    th.check(s.indexOf(new AWTPermission("be.*").toString()) >= 0 ,"checking toString ...");
    th.check(s.indexOf(new AWTPermission("vm.smartmove").toString()) >= 0 ,"checking toString ...");
    th.check(s.indexOf(new AWTPermission("com.acunia.vm*").toString()) >= 0 ,"checking toString ...");

    // boolean implies(Permission permission)
    th.check(pc.implies(bp), "implies -- 1 "+bp);
    bp = new AWTPermission("com.acu*");
    th.check(!pc.implies(bp), "implies -- 2 "+bp);
    bp = new AWTPermission("be.x");
    th.check(pc.implies(bp), "implies -- 3 "+bp);
    bp = new AWTPermission("be");
    th.check(!pc.implies(bp), "implies -- 4 "+bp);
    bp = new AWTPermission("com.acunia");
    th.check(pc.implies(bp), "implies -- 5 "+bp);
    bp = new AWTPermission("com.acunia.vm");
    th.check(!pc.implies(bp), "implies -- 6 "+bp);
    bp = new AWTPermission("vm");
