...
static void next_sequence(x_int current) {
  ...
  if (current >= num_plays) {
    ...
    if (strcmp(result, "the quick brown fox jumps over the lazy dog ") != 0) {
      ...
      runs += 1;
      oempa("Printing thread %d, prio = %d clashes = %d, max competing = %d, max waiting = %d\n", thread_current->id, thread_current->c_prio, sequence_clashes, max_competing, max_waiting);
  ...
  if (Monitor.owner != thread_current) {
    if (thread_current->a_prio != thread_current->c_prio) {
...
static void Ringer(void *r) {
  ...
  oempa("Ringer thread %d '%s' starting...\n", ringer->thread->id, ringer->what);
    ...
    if (! initialized) {
      ...
      for (i = 0; i < NUM_RINGERS; i++) {
        ...
        if (status != xs_success) {
          ...
          oempa("Ringer thread number %d, id = %d\n", i, Ringers[i].thread->id);
        }
      }
      initialized = 1;
    }
...
static void pickup_fork(t_fork afork) {
  ...
  if (afork->monitor.owner != thread_current) {
...
static void Philosopher(void *p) {
  ...
  oempa("Philosopher %d is getting his feet under the table...\n", thread_current->id);
  ...
  while (1) {
    ...
    */
    ...
    if (report) {
      oempa("Thread %d stack: used %d of %d bytes, %d bytes left.\n", thread_current->id, stack_used, stack_size, stack_left);
      oempa("Philosopher %d has eaten %d plates (max = %d, min = %d, ratio = %d)\n", thread_current->id, phil->plates, max_plates, min_plates, ratio);
      report = false;      
      num_reports += 1;
    }      
    ...
    if (! initialized) {
      ...
      for (i = 0; i < number_forks; i++) {
        ...
        if (status != xs_success) {
          ...
          oempa("Philosopher thread %d created.\n", phil->thread->id);
        }
      }
      initialized = 1;
    }
  ...
  while (1) {
    ...
    if (state == 0) {
      ...
      if (status != xs_success) {
        ...
        oempa("Carillon master thread, id %d.\n", c_thread->id);
      }
      ...
      if (status != xs_success) {
        ...
        oempa("Dining philosopher master thread, id %d.\n", p_thread->id);
      }
    ...
    */
    ...
    if (Ringers[NUM_RINGERS - 1].thread != NULL) {
      for (i = 0; i < number_forks; i++) {
        ...
        oempa("%d P %d state = %10s, sticks = %3d, prios %d %d, %d plates\n", i, x->id, x_state2char(x), x->sticks, x->a_prio, x->c_prio, philosophers[i].plates);
      }
      for (i = 0; i < NUM_RINGERS; i++) {
        x = Ringers[i].thread;
    ...
    */
    ...
    x_thread_sleep(second * (x_random() % 5 + 5));
...
static void dump_fsm_threads(void) {
  ...
  for (i = 0; i < num_fsm_threads; i++) {
    ...
    oempa("Mon FSM thread %d (%3d), p a %d c %d, id %3d, fsm p %2d c %2d, state = '%s'\n", i + 1, t->id, t->a_prio, t->c_prio, t->id, mt_Threads[i].ps, mt_Threads[i].cs, x_state2char(t));
  }
...
static void mt_1(void * mta) {
  ...
  while (1) {
    ...
    if (fsm_state == 15) {
      ...
      if (monitor->owner != thread_current) {
        oempa("Bad owner %d != %d\n", monitor->owner->id, thread_current->id);
        dump_fsm_threads();
        exit(0);
      }
      if (monitor->count != 1) {
  ...
  while (1) {
    if (int_state == 0) {
      ...
      */
      ...
      status = x_thread_create(waiter, monitor_waiter, mon, stack, MONT_STACK_SIZE, stopper->c_prio - 1, TF_START);
      if (status != xs_success) {
        oempa("Should be OK!\n");
        exit(0);
      }
      ...
      */
      ...
      oempa("Stop waiting thread, run number %d; this thread has prio %d, waiter has prio %d\n", int_runs, stopper->c_prio, waiter->c_prio);
      int_state = 1;
      status = x_monitor_wait(mon, x_eternal);
      if (status != xs_success) {
        oempa("Should be OK!\n");
...
x_ubyte * monitor_test(x_ubyte * memory) {
  ...
  if (status != xs_success) {
    ...
    oempa("Monitor control thread 1, id = %d\n", control->id);
  }
  ...
  if (status != xs_success) {
    ...
    oempa("Monitor control thread 2, id = %d\n", fsm_thread->id);
  }
