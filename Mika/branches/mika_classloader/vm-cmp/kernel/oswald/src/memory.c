...
/*
** $Id: memory.c,v 1.2 2006/09/11 13:21:39 cvsroot Exp $
**
** This is a memory implementation based on the ideas of Doug Lea's malloc. 
**
** The most significant features/changes are:
**
** - change of previous in use bit
** - addition of a current in use bit
** - all sizes are recorded and calculated as double word sizes
** - the maximum single chunk that can be allocated is 8 Mb.
** - the saved bits of limiting the size and using double word calculations, are used for implementing user defined tags
** - I implemented a memory walker function that can walk over all allocated chunks, checking for tags f.i. and a scan
**   function for finding chunks with a certain tag.
** - a free function that just tags the chunk as garbage is provided, together with a simple garbage collection function.
**   these allow very quick 'discarding' of memory for later garbage collection.
** - there is a function x_mem_is_block that checks wether a value passed as argument is a heap pointer
**   allocated through one of the allocation functions. Through the use of an indexing array, this lookup is
**   quite fast. Keeping the index up to date requires only a small amount of extra work.
**
** NOTES :
**
** This is not hacked code, but a completely new implementation *based* on some principles of Dougs malloc. I
** hereby would like to thank Doug for making his implementation of malloc public.
**
** This code is *VERY* tricky to change/hack. If you change something, be sure to run the tests for
** a significant amount of time and this with all checks enabled.
**
** TODO: when free space is exhausted, start our own garbage collector and retry...
*/
...
static void * x_sbrk(x_bytes bytes) {
  ...
  if (bytes > 0 && allocated > max_heap_bytes) {
    loempa(9, "No more memory: %d Mbytes used.\n", allocated / (1024 * 1024));
    return NULL;
  }  
...
static void _check_monitor(int l, x_size count) {
  if (memory_monitor->count != count && memory_monitor->owner != thread_current) {
    loempa(9, "%d: wrong count %d != %d owner = 0x%08x current = 0x%08x\n", l, memory_monitor->count, count, memory_monitor->owner, thread_current);
    abort();
  }
}

#define memory_lock(m) _memory_lock(__LINE__, m)

inline static void _memory_lock(x_int line, x_monitor m) {

  x_status status;
  
  status = x_monitor_eternal(m);
  if (status != xs_success) {
    loempa(9, "%d: bad status '%s'\n", line, x_status2char(status));
    abort();
  }

}

#define memory_unlock(m) _memory_unlock(__LINE__, m)

inline static void _memory_unlock(x_int line, x_monitor m) {
...
inline static x_chunk _next_chunk(int l, x_chunk chunk) {
  ...
  if (chunk->this_size & CURRENT_IN_USE) {
    loempa(9, "%d: Chunk 0x%08x - 0x%08x is in use and next chunk is taken !!\n", l, chunk, chunk->this_size);
    abort();
  }
...
inline static x_units _previous_size(int l, x_chunk chunk) {
  ...
  if (chunk->this_size & PREVIOUS_IN_USE) {
    loempa(9, "%d: previous size field taken for a chunk that is in use.!\n", l);
    abort();
  }
...
inline static x_word _is_inuse(int l, x_chunk chunk) {
   ...
   if (! (chunk->this_size & CURRENT_IN_USE) && chunk != top) {
     if (treat_as_chunk(chunk, chunk->this_size & FREE_UNITS_MASK)->this_size & PREVIOUS_IN_USE) {
       loempa(9, "%d: Wrong 0x%08x. Memory is corrupt!\n", l, chunk);
       abort();
     }
   }
   if (chunk->this_size & CURRENT_IN_USE && chunk != top) {
...
inline static x_units _chunk_size(int l, x_chunk chunk) {
  ...
  if (chunk->this_size & CURRENT_IN_USE && ! (chunk->this_size & BIG_CHUNK)) {
    loempa(9, "%d: getting size of in use chunk !\n", l);
    abort();
  }
...
inline static void _set_size_set_piu(int l, x_chunk chunk, x_units size) {
  ...
  */
  ...
  if ((chunk->this_size & CURRENT_IN_USE) && (size & ~USED_UNITS_MASK) != 0) {
    loempa(9, "%d: size = 0x%08x (%d Mb)\n", l, size, (size << 3) / (1024 * 1024));
    abort();
  }

  if (size < 0) {
    loempa(9, "Size is negative!\n");
    abort();
  }

  if (chunk->this_size & CURRENT_IN_USE) {
    loempa(9, "%d: chunk is in use\n", l);
    abort();
  }
...
static void _check_top(int l, x_chunk chunk) { 
  ...
  if (is_inuse(chunk)) {
    loempa(9, "%d: top isn't supposed to be in use!\n", l);
    abort();
  }
  
  if (chunk_size(chunk) < minimum_size) {
    loempa(9, "%d: top size smaller then minimum %d < %d!\n", l, chunk_size(chunk), minimum_size);
    abort();
  }

  if (! aligned_OK(chunk)) {
    loempa(9, "%d: top chunk doesn not align properly.\n", l);
    abort();
  }
...
static void _check_inuse_chunk(int l, x_chunk chunk) { 
  ...
  if (! chunk->this_size & CURRENT_IN_USE) {
    loempa(9, "Chunk should be in use!\n");
    abort();
  }
...
static void _check_alloced_chunk(int l, x_chunk chunk, x_units requested) { 
  ...
  if (chunk->this_size & TAG_MASK) {
    loempa(9, "%d: tag bits are not 0.\n", l);
    abort();
  }

  if (chunk->this_size & GARBAGE_TAG) {
    loempa(9, "%d: garbage bit is not 0.\n", l);
    abort();
  }
...
void * x_mem_alloc(x_size request) {
...
void * x_mem_calloc(x_size request) {
  ...
  */
  ...
  for (cursor = initial; cursor < top; cursor = next) {
    ...
    if (runtime_checks && size == 0) {
      loempa(9, "Size is 0; cursor = 0x%08x\n", cursor);
      abort();
    }
  }
