/*******************************************************************************
 * Copyright (c) 2001 by Acunia N.V. All rights reserved. * * This software is
 * copyrighted by and is the sole property of Acunia N.V. * and its licensors,
 * if any. All rights, title, ownership, or other * interests in the software
 * remain the property of Acunia N.V. and its * licensors, if any. * * This
 * software may only be used in accordance with the corresponding * license
 * agreement. Any unauthorized use, duplication, transmission, * distribution or
 * disclosure of this software is expressly forbidden. * * This Copyright notice
 * may not be removed or modified without prior * written consent of Acunia N.V. * *
 * Acunia N.V. reserves the right to modify this software without notice. * *
 * Acunia N.V. * Vanden Tymplestraat 35 info@acunia.com * 3000 Leuven
 * http://www.acunia.com * Belgium - EUROPE *
 ******************************************************************************/
...
public class Collections {
   ...
   * @author ruelens created: Feb 5, 2007
   */
  public static class FinalMapEntrySet extends AbstractSet {

    private Set back;

    public FinalMapEntrySet(Set set) {
      back = set;
    }

    public Iterator iterator() {
      return new EntryIterator(back.iterator());
    }
  ...
  public static int binarySearch(List ls, Object e, Comparator comp) {
    ...
    if(ls instanceof RandomAccess) {
      ...
      int cmp;
      do {
        cmp = comp.compare(ls.get(index),e);
        if(cmp == 0) {
          return index;          
        }
        if(cmp > 0) {
    ...
    for (int i = 0; i < size; i++) {
      int cmp = comp.compare(iter.next(), e);
      if (cmp == 0) {
        return i;
      }
      if (cmp > 0) {
  ...
  /**
   * * shuffle the list using the given random. * this method run a for loop...
   * (size times) * it switches the current pos with a random one * this means
   * every index is at least one time involved in a * involved in a switching
   * operation
   */
  ...
  private static class FinalMap extends AbstractMap implements Serializable {
    ...
    public Set entrySet() {
      return new FinalMapEntrySet(back.entrySet());
    }
  ...
  private static class FinalSortedMap extends AbstractMap implements SortedMap,
      Serializable {
  ...
  private static class SyncedList extends SyncedCollection implements List,
      Serializable {
    private List list;
  ...
  private static class SyncedSet extends SyncedCollection implements Set,
      Serializable {
